...
// Fechar modais ao clicar fora (na área escura)
...
// Fechar modais ao apertar a tecla ESC no teclado
...
function realizarLogin() {
    ...
    setTimeout(() => {
        ...
        // Puxa o usuário do banco carregado ou verifica se é o master local
...
function renderizarListaEstoque() {
    ...
    dadosEstoqueGlobal.forEach(row => { 
        let nomeLojaSheet = (row["Loja"] || "").trim().toLowerCase();
        ...
        if (lojaEncontrada) {
            ...
            if (chave && mapaEmojis[chave]) {
                ...
                if (lojasEstoque[lojaEncontrada][nomeOficial] !== undefined) {
                    lojasEstoque[lojaEncontrada][nomeOficial] += Number(row["Quantidade"]) || 0;
                }
            }
        } 
    });
...
// CORREÇÃO: LÊ EXATAMENTE O NOME DO MODELO SEM CONFUNDIR A6, A6X e A6 PRO
...
function renderizarListaHistorico() {
    ...
    let filtrados = dadosHistoricoGlobal.filter(row => {
        ...
        if(dataInicio && rowData) { 
            let dtStr = rowData;
            if(dtStr.includes("/")) { let parts = dtStr.split(" ")[0].split("/"); if(parts.length === 3) dtStr = `${parts[2]}-${parts[1]}-${parts[0]}`; }
            let dt = new Date(dtStr); if(dt < new Date(dataInicio + "T00:00:00")) return false; 
        }
        if(dataFim && rowData) { 
            let dtStr = rowData;
            if(dtStr.includes("/")) { let parts = dtStr.split(" ")[0].split("/"); if(parts.length === 3) dtStr = `${parts[2]}-${parts[1]}-${parts[0]}`; }
            let dt = new Date(dtStr); if(dt > new Date(dataFim + "T23:59:59")) return false; 
        }
    ...
    let html = "";
    filtrados.forEach(row => {
        let dataFormatada = getVal(row, ['datahora', 'data', 'timestamp', 'carimbo']) || "Sem Data";
        if (dataFormatada !== "Sem Data") {
            try {
                ...
                else if(dataFormatada.includes("T")) { let dx = new Date(dataFormatada); dataFormatada = dx.toLocaleDateString('pt-BR') + ' às ' + dx.toLocaleTimeString('pt-BR', {hour: '2-digit', minute:'2-digit'}); }
            } catch(e) {}
        }
...
function gerarGraficos(dadosVendas) {
    ...
    dadosVendas.forEach(row => {
        ...
        for (let k in bancoUsuarios) {
            let u = bancoUsuarios[k];
            if (u.cargo !== "promotor") continue;
            
            // BLINDAGEM MÁXIMA: Aceita se bater a loja exata na lista OU se o nome do vendedor combinar com o do promotor
            let storeMatches = (loja !== "Outras" && u.lojasPermitidas && u.lojasPermitidas.some(l => l.trim().toLowerCase() === loja.toLowerCase()));
            let nomeLimpoVend = vendNome.toLowerCase().trim();
            let nomeLimpoU = (u.nome || "").toLowerCase().trim();
            let loginLimpo = k.toLowerCase().trim();
            let nameMatches = (nomeLimpoVend && nomeLimpoU && (nomeLimpoVend.includes(nomeLimpoU) || nomeLimpoU.includes(nomeLimpoVend) || nomeLimpoVend.includes(loginLimpo) || loginLimpo.includes(nomeLimpoVend)));
            
            if (storeMatches || nameMatches) {
                let passaFiltroSup = (supervisorFoco === "todos" || u.criadoPor === supervisorFoco);
                let passaFiltroProm = (promotorFoco === "todos" || k === promotorFoco);
                
                if (passaFiltroSup && passaFiltroProm) {
                    if (usuarioLogado.cargo === "promotor") {
                        if (k === usuarioLogado.id) {
                            pertenceAoEscopo = true; promotoresImpactados.add(k); if(u.criadoPor) supervisoresImpactados.add(u.criadoPor);
                        }
                    } else if (podeGerenciar(usuarioLogado, k)) {
                        pertenceAoEscopo = true; promotoresImpactados.add(k); if(u.criadoPor) supervisoresImpactados.add(u.criadoPor); 
                    }
                }
            }
