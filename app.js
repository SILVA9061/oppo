...
// === INICIALIZAÇÃO SEGURA (CÓDIGO ORIGINAL QUE FUNCIONA) ===
...
// NOVA FUNÇÃO ESSENCIAL PARA O DASHBOARD (Hierarquia)
...
function carregarNotificacoesSilencioso() {
    fetch(URL_DA_SUA_API + "?acao=historico&limit=150" + noCache())
    .then(r => r.json())
    .then(res => {
...
function renderizarInputsFoco() {
    ...
    let htmlNiveis = `<div style="background: var(--bg-item); padding: 15px; border-radius: 8px; border: 1px solid var(--border-color); margin-top: 15px; text-align: left;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
            <span style="font-size: 13px; font-weight: bold; color: #6f42c1; display:flex; align-items:center;"><i data-lucide="layers" class="lucide-sm"></i> Estrutura de Níveis de Comissão</span>
        ...
        <p style="font-size: 11px; color: var(--cor-secundaria); margin-bottom: 10px;">Defina a <strong>Meta Mínima de Vendas Gerais</strong> que o promotor deve atingir no mês para destravar cada nível.</p>
    ...
    niveisGlobais.forEach((nv, idx) => {
        ...
        <div class="linha-nivel-config" data-id="${nv.id}" style="display:flex; align-items:center; gap:10px; background:var(--bg-container); padding:8px; border-radius:6px; border: 1px solid var(--border-color);">
            ...
            <span style="font-size:11px; color:var(--cor-secundaria);">Meta (un):</span>
            ...
            ${idx > 0 ? `<button class="btn-excluir" onclick="removerNivelGlobal('${nv.id}')" style="padding:4px 8px; margin-left:auto;"><i data-lucide="x"></i></button>` : `<span style="margin-left:auto; font-size:10px; color:#28a745;">(Padrão)</span>`}
        </div>`;
    });
    htmlNiveis += `</div></div>`;
    ...
    let htmlAparelhos = `<div style="font-size: 13px; font-weight: bold; color: var(--cor-texto); margin: 20px 0 10px 0; text-align: left; display:flex; align-items:center;"><i data-lucide="smartphone" class="lucide-sm"></i> Valores de Comissão por Aparelho:</div>
    ...
    for (let ap in mapaEmojis) {
        ...
        niveisGlobais.forEach(nv => {
            ...
            <div style="min-width: 75px;">
                <span style="font-size: 10px; font-weight: bold; color: var(--cor-secundaria);">${nv.nome} (R$)</span>
                <input type="number" class="input-comissao-nivel" data-ap="${ap}" data-lvl="${nv.id}" value="${v}" style="width: 100%; padding: 6px; margin-top: 4px; background: var(--bg-input); color: var(--cor-texto); font-size: 12px; border-radius: 4px; border: 1px solid var(--border-color);" onchange="atualizarListaPremiumGlobal()">
            </div>`;
        });
